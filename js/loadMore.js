$(document).ready(function(){
    ...
    $(".loadMoreBtn").on('click', function (e) { //user clicks on button
        ...
        if($(this).hasClass('loadMoreBtnHome')){
            ...
            query = "SELECT * FROM posts WHERE topics_ID not in (SELECT topics_ID FROM users_topics WHERE users_ID NOT IN (SELECT id FROM users WHERE email = :email)) ORDER BY id DESC LIMIT :position, :limit";
        }
        load_contents(track_page, query); //load content
    });
